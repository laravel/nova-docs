...
#### 3-Way Relationships
In some cases you may have a 3 way `pivot` table where you wan't to display only the related records. 
...
where you want to display the guests with their room number in the reservation show page.

In order to achieve this you will need to define a [definine a custom intermediate Table Model](https://laravel.com/docs/8.x/eloquent-relationships#defining-custom-intermediate-table-models). And the the `belongsTo` method.

In our case:
```php
class RoomReservationGuest extends \Illuminate\Database\Eloquent\Relations\Pivot
{
...
```
[Create a resource](./../resources/#defining-resources) for the `RoomReservationGuest` model and define it's fields.
...
Define a `HasMany` field in the `Reservation` resource for the `RoomReservationGuest` relation.
```php
public function fields(Request $request)
{
    return [
