...
* Support Inertia Laravel 1.3+ and 2.0+
...
```php app/Nova/NovaServiceProvider.php
/**
 * Register the configurations for Laravel Fortify.
 */
protected function fortify(): void
{
    Nova::fortify()->register();
}
...
With Nova 5, the frontend JavaScript now utilizes Inertia.js v2 and will affect any projects that directly import from `@inertiajs/inertia` or `@inertiajs/inertia-vue3`. You need to inspect your components/packages and ensure all references have been updated as suggested in [Inertia's Upgrade Guide](https://inertiajs.com/upgrade-guide).
...
### Deprecate Form Abandonment (using Browser navigation)

With the introduction of Inertia.js v2, we no longer able to provide the feature using the available API from Inertia.js to override `popstate` event. For Nova 5, using the browser's back button or clicking on `Cancel` will no longer prompt a confirmation dialog.
