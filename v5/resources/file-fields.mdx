...
You may also use the `maxWidth`, `indexWidth`, and `detailWidth` methods on the [Avatar](#avatars) and [Gravatar](/v5/resources/fields#gravatar-field) fields.
</Note>
...
Second argument `$requestAttribute` contains the exact payload path to the File field in the request. Most convenient for use within [Repeater](/v5/resources/repeater-fields#overview).
